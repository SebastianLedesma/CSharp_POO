...
namespace WindowsForms.SP
{
    ...
    public partial class SegundoParcial : Form
    {
        ...
        private void btnPunto4_Click(object sender, EventArgs e)
        {
            ...
            this.c_gomas.EventoPrecio += new DelegadoCartucheraCostosa(this.c_gomas_EventoPrecio);
            this.c_gomas += new Goma(false, "Faber-Castell", 31);
        }

        private void c_gomas_EventoPrecio(object sender, EventArgs e)
        {
            string ruta = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments).ToString() + "\\tickets.log";
            string cadena = DateTime.Now + " \n" + "\nPrecio cartuchera:"+ ((Cartuchera<Goma>)sender).PrecioTotal + "\n";

            bool todoOK = Tickeadora.ImprimirTicket(ruta,cadena);//Reemplazar por la llamada al método de clase ImprimirTicket
        ...
        private void btnVerLog_Click(object sender, EventArgs e)
        {
            ...
            DialogResult rta = this.openFileDialog1.ShowDialog();//DialogResult.Cancel;//Reemplazar por la llamada al método correspondiente del OpenFileDialog
            ...
            if (rta == DialogResult.OK)
            {
                ...
                StreamReader st = new StreamReader(lector);

                this.txtVisorTickets.Text = st.ReadToEnd();
                st.Close();
        ...
        private void btnPunto6_Click(object sender, EventArgs e)
        {
            ...
            this.cn = new SqlConnection(@"Server=localhost\SQLEXPRESS;Database=utilesVersion2;Trusted_Connection=True;");
        ...
        private void btnPunto7_Click(object sender, EventArgs e)
        {
            ...
            //this.cn = new SqlConnection(Properties.Settings.Default.conexionUtiles);
            ...
            foreach (Lapiz item in this.c_lapices.Elementos)
            {
                fila = this.dt.NewRow();
            ...
            foreach (Goma item in this.c_gomas.Elementos)
            {
                fila = this.dt.NewRow();
        ...
        private void btnPunto9_Click(object sender, EventArgs e)
        {
            ...
            //this.cn = new SqlConnection(Properties.Settings.Default.conexionUtiles);
            this.da.UpdateCommand = new SqlCommand("UPDATE FROM utiles SET marca = @marca, precio = @precio, tipo = @tipo WHERE id = @id",this.cn);
